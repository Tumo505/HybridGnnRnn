...
def create_graph_targets(batch):
    """Create graph-level targets from node-level targets."""
    ...
    return graph_class_targets, graph_reg_targets
...
def evaluate_model(model, data_loader, device, criterion_class, criterion_reg):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            graph_class_targets, graph_reg_targets = create_graph_targets(batch)
    ...
    class_accuracy = accuracy_score(all_class_targets, all_class_preds) if all_class_targets else 0.0
    class_f1 = f1_score(all_class_targets, all_class_preds, average='weighted') if all_class_targets else 0.0
...
def main():
    ...
    try:
        ...
        model = SpatialGNN(
            ...
            use_batch_norm=False  # Disable batch norm to avoid single-graph batch issues
        ).to(device)
